...
- (id)init
{
	if(self = [super init])
	{
        ...
        NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:10 target:self selector:@selector(timerSend:) userInfo:nil repeats:YES];
 
	}
	return self;
}

- (void)timerSend:(id)sender
{
    NSString *welcomeMsg = [NSString stringWithFormat:@"定时器测试 %@",[NSDate date]];
    ...
    for(int i = 0; i < [connectedSockets count]; i++)
    {
        ...
        [[connectedSockets objectAtIndex:i] writeData:welcomeData withTimeout:-1 tag:WELCOME_MSG];
    }
